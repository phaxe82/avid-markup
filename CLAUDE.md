...
2. **Align** (`whisperx.align`, Whisper path only) and **diarize** (`pyannote`) — both run on `_pick_torch_device()` (MPS if available, else CPU). **Diarization dominates runtime on long files**, so getting it onto MPS is the single biggest speedup (~47 min → ~6 min on a 47-min file). For the Parakeet path, speakers are assigned by time-overlap (`_assign_speakers_by_overlap`) since whisperx's word-level `assign_word_speakers` doesn't apply.
